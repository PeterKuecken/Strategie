...
function renderSingleChapter(s,c,idx){
  ...
  html+=`</div>`;
  const prev=idx>0?`<button class="copy-btn" onclick="openChapter(${idx-1})">Vorheriger Eintrag</button>`:'';
  const next=idx<(s.chapters.length-1)?`<button class="copy-btn" onclick="openChapter(${idx+1})">Nächster Eintrag</button>`:'';
  let status='';
...
function renderProgressOverview(){return `<div class="card"><h3>Fortschritt Jahrespläne</h3>${progressBar('linkedin52','LinkedIn Jahresplan')}${progressBar('facebook52','Facebook Jahresplan')}${progressBar('videos52','Video Jahresplan')}</div>`}
function renderProgressOverviewForSection(id){if(id==='linkedin52')return `<div class="card">${progressBar(id,'LinkedIn Jahresplan')}</div>`; if(id==='facebook52')return `<div class="card">${progressBar(id,'Facebook Jahresplan')}</div>`; if(id==='videos52')return `<div class="card">${progressBar(id,'Video Jahresplan')}</div>`; return ''}
